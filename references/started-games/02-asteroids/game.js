...
let score, lives, level, powerupSpawned;
...
function update(dt) {
  ...
  // Bala vs asteroide
  const newAsteroids = [];
  let lastKilledX = W / 2, lastKilledY = H / 2;
  for (const b of bullets) {
    for (const a of asteroids) {
      if (!a.dead && !b.dead && dist(b, a) < a.radius) {
        ...
        if (!powerupSpawned && Math.random() < POWERUP_SPAWN_CHANCE) {
  ...
  const afterFilter = asteroids.filter(a => !a.dead).concat(newAsteroids);
  if (afterFilter.length === 0 && !powerupSpawned && powerups.length === 0) {
    powerups.push(new PowerUp(lastKilledX, lastKilledY));
    powerupSpawned = true;
  }
  asteroids = afterFilter;
  bullets   = bullets.filter(b => !b.dead);
...
function drawHUD() {
  ...
  if (ship.tripleShotTimer > 0) {
    ...
    ctx.textAlign = 'left';
    ctx.fillText(`3\xd7  ${Math.ceil(ship.tripleShotTimer)}s`, 14, 48);
  }
}
